...
class BaseEntityModel(rg.Map):
    ...
    def render(self):
        ...
        if not tmap:
          ...
          for i in range(64):
            tmap.append([0,0,0])
        if [self.mx, self.my, self.mz, self.world] == [super().cmx, super().cmy, super().cmz, super().cworld] and not [self.x, self.y, self.z, self.world, self.mx, self.my, self.mz] == [self.lx, self.ly, self.lz, self.lworld, self.lmx, self.lmy, self.lmz]:
